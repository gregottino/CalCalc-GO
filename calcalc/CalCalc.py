...
def calculate(input_str, wolfram_switch=False):
    ...
    # if numexpr fails, see if wolfram output was desired
        ...
        # if wolfram output is desired,
        # query the API for an answer and print it
        if wolfram_switch:
            # put the query into html format
            url_friendly_input_string = urllib.parse.quote_plus(input_str)
            # open the URL and parse it
            try:
                output_str = urllib.request.urlopen(
                    f"http://api.wolframalpha.com/v2/result?i={url_friendly_input_string}&appid={wolfram_app_id}&format=plaintext"
                ).read()
...
def test_4():
    ...
    assert get_wolfram_output_as_float(calculate(test_str, True)) == 7.3459e22
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "input_str",
        help="Input string of mathematical expression, to be evaluated. Arguement should be a simple string or quote enclosed string",
    )
    parser.add_argument(
        "-w",
        action="store_true",
